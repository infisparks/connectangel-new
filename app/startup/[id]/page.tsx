...
// **FIXED INTERFACE HERE** - Corrected casing
...
export default function StartupInfoPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="bg-[#1B0E2B] min-h-screen py-8 font-sans text-white">
      ...
      <div className="max-w-[1438px] mx-auto px-4 sm:px-6 lg:px-10 mt-24">

        {/* Hero Section */}
        <div className="relative w-full rounded-[25px] overflow-hidden p-6 sm:p-10 text-center flex flex-col items-center border border-white/10 bg-white/[0.03] backdrop-blur-3xl shadow-lg mb-8">
          ...
          <h1 className="text-5xl sm:text-6xl font-extrabold mb-2 relative z-10">{startup.startup_name}</h1>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4 relative z-10">
            {startup.is_incubation && startup.incubation_id && (
              <Link href={`/incubation-dashboard/${startup.incubation_id}`}>
                <div className="px-5 py-2 border border-purple-500 bg-purple-500/20 text-purple-200 rounded-full text-sm sm:text-base font-semibold transition-all duration-300 hover:bg-purple-500/40 cursor-pointer animate-glow">
                  {startup.incubator_accelerator_name || "Incubation"}
                </div>
              </Link>
            )}
            <div className="px-5 py-2 border border-white/20 rounded-full text-sm sm:text-base font-semibold">{startup.domain || "AI/ML"}</div>
            <div className="px-5 py-2 border border-white/20 rounded-full text-sm sm:text-base font-semibold">{startup.establishment_year || "2022"}</div>
            <div className="px-5 py-2 border border-white/20 rounded-full text-sm sm:text-base font-semibold">{startup.location || "Maharashtra"}</div>
            <div className="px-5 py-2 border border-white/20 rounded-full text-sm sm:text-base font-semibold">{startup.funding_stage || "Series A"}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          <div className="p-8 bg-white/5 rounded-3xl border border-white/10">
            ...
            <p className="text-gray-300 mb-6">{startup.description || "We turn your smart home vision into reality with AI-powered IoT solutions that are private, secure, and delightful to use."}</p>
          ...
          <div className="flex flex-col gap-8">
            <div className="p-8 bg-white/5 rounded-3xl border border-white/10">
              ...
              <p className="text-gray-300">{startup.problem_being_solved || "Homes rely on multiple vendor apps that don't work together, creating friction and security bottlenecks."}</p>
            </div>
            <div className="p-8 bg-white/5 rounded-3xl border border-white/10">
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              {startup.team_members.map((member, index) => (
                <div
                  key={index}
                  className="bg-white/5 rounded-2xl p-6 flex flex-col items-center text-center border border-white/10"
                >
        ...
        <div className="p-8 bg-[#1B0E2B] rounded-[25px] border border-white/10 flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
          <div className="text-center md:text-left">
            <h3 className="text-3xl font-semibold mb-2">Interested in {startup.startup_name}?</h3>
            <p className="text-gray-400">Request the deck or schedule a demo.</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
            <Link href={startup.website_url || "#"} target="_blank">
              <Button
                variant="outline"
                className="w-full sm:w-auto px-8 py-6 rounded-full text-base font-bold text-white border-white/50 hover:bg-white/10 transition-colors"
              >
                Visit Website
              </Button>
            </Link>
            <Link href={`mailto:${startup.email_address || "#"}`}>
              <Button
                className="w-full sm:w-auto bg-purple-600 text-white rounded-full px-8 py-6 text-base font-bold transition-colors hover:bg-purple-700"
              >
                Get In Touch
              </Button>
            </Link>
        ...
        {similarStartups.length > 0 && (
          <div className="mb-8">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-3xl font-bold">Startups Solving Similar Problems</h2>
        ...
        <div>
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-3xl font-bold">Free Training Series</h2>
            <div className="group w-12 h-12 border-2 border-white/50 rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 hover:bg-white/10 hover:border-white">
              <ArrowRightIcon className="w-6 h-6 text-white/80 transition-all duration-300 group-hover:text-white" />
            </div>
          </div>
