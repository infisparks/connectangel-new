...
export default function MyProfilesPage() {
  ...
  const [activeMainTab, setActiveMainTab] =
    useState<ProfileRoleType>("startup"); // State for main tabs
  ...
  const fetchMyProfiles = useCallback(async () => {
    ...
    if (!user) {
      setError(
        "You must be logged in to view your profiles. Redirecting to login."
      );
      setLoading(false);
      router.push("/register");
      return;
    ...
    const { data: pendingNeedsUpdateRejectedStartupData, error: pendingNeedsUpdateRejectedStartupError } = await supabase
    ...
    if (pendingNeedsUpdateRejectedStartupData) {
      setPendingStartups(pendingNeedsUpdateRejectedStartupData.filter(p => p.status === 'pending') as PendingStartup[]);
      setNeedsUpdateStartups(pendingNeedsUpdateRejectedStartupData.filter(p => p.status === 'needs_update') as PendingStartup[]);
      setRejectedStartups(pendingNeedsUpdateRejectedStartupData.filter(p => p.status === 'rejected') as PendingStartup[]); // Separated rejected
    } else if (pendingNeedsUpdateRejectedStartupError) {
      console.error("Error fetching pending/needs_update/rejected startups:", pendingNeedsUpdateRejectedStartupError.message);
      setError((prev) => (prev ? prev + "\n" : "") + "Error loading pending/needs_update/rejected startups: " + pendingNeedsUpdateRejectedStartupError.message);
    }
    ...
    const { data: pendingNeedsUpdateRejectedIncubationData, error: pendingNeedsUpdateRejectedIncubationError } = await supabase
    ...
    if (pendingNeedsUpdateRejectedIncubationData) {
      ...
      setRejectedIncubations(pendingNeedsUpdateRejectedIncubationData.filter(p => p.status === 'rejected') as IncubationProfile[]); // Separated rejected
    } else if (pendingNeedsUpdateRejectedIncubationError) {
      console.error("Error fetching pending/needs_update/rejected incubations:", pendingNeedsUpdateRejectedIncubationError.message);
      setError((prev) => (prev ? prev + "\n" : "") + "Error loading pending/needs_update/rejected incubations: " + pendingNeedsUpdateRejectedIncubationError.message);
    }
    ...
    const { data: pendingNeedsUpdateRejectedInvestorData, error: pendingNeedsUpdateRejectedInvestorError } = await supabase
    ...
    if (pendingNeedsUpdateRejectedInvestorData) {
      ...
      setRejectedInvestors(pendingNeedsUpdateRejectedInvestorData.filter(p => p.status === 'rejected') as InvestorProfile[]); // Separated rejected
    } else if (pendingNeedsUpdateRejectedInvestorError) {
      console.error("Error fetching pending/needs_update/rejected investors:", pendingNeedsUpdateRejectedInvestorError.message);
      setError((prev) => (prev ? prev + "\n" : "") + "Error loading pending/needs_update/rejected investors: " + pendingNeedsUpdateRejectedInvestorError.message);
    }
    ...
    if (pendingNeedsUpdateRejectedMentorData) {
      ...
      console.error("Error fetching pending/needs_update/rejected mentors:", pendingNeedsUpdateRejectedMentorError.message);
      setError((prev) => (prev ? prev + "\n" : "") + "Error loading pending/needs_update/rejected mentors: " + pendingNeedsUpdateRejectedMentorError.message);
    }
      ...
      <Tabs value={activeMainTab} onValueChange={(value) => setActiveMainTab(value as ProfileRoleType)} className="w-full">
        <TabsList className="grid w-full grid-cols-4 bg-gray-800 text-gray-300">
          <TabsTrigger value="startup" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white">
            Startups ({approvedStartups.length + pendingStartups.length + needsUpdateStartups.length + rejectedStartups.length})
          </TabsTrigger>
          <TabsTrigger value="incubation" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white">
            Incubations ({approvedIncubations.length + pendingIncubations.length + needsUpdateIncubations.length + rejectedIncubations.length})
          </TabsTrigger>
          <TabsTrigger value="investor" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white">
            Investors ({approvedInvestors.length + pendingInvestors.length + needsUpdateInvestors.length + rejectedInvestors.length})
          </TabsTrigger>
          <TabsTrigger value="mentor" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white">
            Mentors ({approvedMentors.length + pendingMentors.length + needsUpdateMentors.length + rejectedMentors.length})
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="startup" className="mt-6">
          <Tabs defaultValue="approved-startups" className="w-full">
            <TabsList className="grid w-full grid-cols-4 bg-gray-800 text-gray-300"> {/* Changed grid-cols to 4 */}
              ...
              <TabsTrigger value="needs-update-startups" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white">
                Needs Update ({needsUpdateStartups.length})
              </TabsTrigger>
              <TabsTrigger value="rejected-startups" className="data-[state=active]:bg-red-600 data-[state=active]:text-white"> {/* New Rejected tab */}
            ...
            <TabsContent value="rejected-startups" className="mt-6"> {/* New Rejected content */}
        ...
        <TabsContent value="incubation" className="mt-6">
          <Tabs defaultValue="approved-incubations" className="w-full">
            <TabsList className="grid w-full grid-cols-4 bg-gray-800 text-gray-300"> {/* Changed grid-cols to 4 */}
              ...
              <TabsTrigger value="needs-update-incubations" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white">
                Needs Update ({needsUpdateIncubations.length})
              </TabsTrigger>
              <TabsTrigger value="rejected-incubations" className="data-[state=active]:bg-red-600 data-[state=active]:text-white"> {/* New Rejected tab */}
            ...
            <TabsContent value="rejected-incubations" className="mt-6"> {/* New Rejected content */}
        ...
        <TabsContent value="investor" className="mt-6">
          <Tabs defaultValue="approved-investors" className="w-full">
            <TabsList className="grid w-full grid-cols-4 bg-gray-800 text-gray-300"> {/* Changed grid-cols to 4 */}
              ...
              <TabsTrigger value="needs-update-investors" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white">
                Needs Update ({needsUpdateInvestors.length})
              </TabsTrigger>
              <TabsTrigger value="rejected-investors" className="data-[state=active]:bg-red-600 data-[state=active]:text-white"> {/* New Rejected tab */}
            ...
            <TabsContent value="rejected-investors" className="mt-6"> {/* New Rejected content */}
        ...
        <TabsContent value="mentor" className="mt-6">
          <Tabs defaultValue="approved-mentors" className="w-full">
            <TabsList className="grid w-full grid-cols-4 bg-gray-800 text-gray-300"> {/* Changed grid-cols to 4 */}
              ...
              <TabsTrigger value="needs-update-mentors" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white">
                Needs Update ({needsUpdateMentors.length})
              </TabsTrigger>
              <TabsTrigger value="rejected-mentors" className="data-[state=active]:bg-red-600 data-[state=active]:text-white"> {/* New Rejected tab */}
            ...
            <TabsContent value="rejected-mentors" className="mt-6"> {/* New Rejected content */}
