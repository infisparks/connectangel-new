// app/incubation-registration/page.tsx
...
import { IncubationMultiStepForm } from "./IncubationMultiStepForm"; // Adjust this path if needed
import { toast } from "sonner"; // For notifications

/**
 * Renders the Incubation Registration Page.
 * This is a client component responsible for fetching the user ID
 * and then rendering the multi-step incubation form.
 */
export default function IncubationRegistrationPage() {
  ...
  useEffect(() => {
    async function getSession() {
      ...
      if (data?.session) {
        ...
        toast.info("Please log in to register your incubation profile.");
    ...
    getSession();
  }, [router]); // Depend on router to ensure effect runs if router changes (though unlikely for a page component)
  ...
  return (
    <IncubationMultiStepForm userId={userId} />
  );
}