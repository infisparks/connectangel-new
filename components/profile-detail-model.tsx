...
import {
  ...
  import {
    ...
    ProfileData,
    ProfileRoleType,
  } from "@/app/my-startups/page"; // Adjust import path as needed
  ...
  const renderDetailRow = (label: string, value: string | number | string[] | undefined | boolean | React.ReactElement) => {
    ...
    return (
      <div className="grid grid-cols-2 gap-4 py-2">
        ...
        <div className="text-gray-200">
          {React.isValidElement(value) ? value : Array.isArray(value) ? value.join(", ") : typeof value === 'boolean' ? (value ? 'Yes' : 'No') : value}
        </div>
      </div>
    );
  };
  ...
  export function ProfileDetailModal({
    ...
    const renderStartupDetails = (startup: ApprovedStartup | PendingStartup) => (
      <>
        ...
        {renderDetailRow("Pitch Video URL", startup.pitch_video_url && <a href={startup.pitch_video_url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">{startup.pitch_video_url}</a>)}
        {renderDetailRow("Thumbnail URL", startup.thumbnail_url && <a href={startup.thumbnail_url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">{startup.thumbnail_url}</a>)}
        {renderDetailRow("Status", startup.status)}
        {'approved_at' in startup && startup.approved_at && renderDetailRow("Approved At", new Date(startup.approved_at).toLocaleDateString())}
        ...
        {renderDetailRow("Instagram", startup.instagram_url && <a href={startup.instagram_url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Link</a>)}
        {renderDetailRow("LinkedIn", startup.linkedin_url && <a href={startup.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Link</a>)}
        {renderDetailRow("Website", startup.website_url && <a href={startup.website_url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Link</a>)}
            ...
            <ul className="list-disc list-inside text-gray-200">
              {startup.team_members.map((member, index) => (
                <li key={index}>
                  ...
                  {member.phoneCountryCode} {member.localPhoneNumber}
                </li>
              ))}
            </ul>
          </div>
    ...
    const renderIncubationDetails = (incubation: IncubationProfile) => (
      <>
        ...
        {renderDetailRow("Website", incubation.website && <a href={incubation.website} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Link</a>)}
        {renderDetailRow("LinkedIn", incubation.linkedin_profile && <a href={incubation.linkedin_profile} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Link</a>)}
        ...
        {renderDetailRow("Percentage Startups Operational After 3 Yrs", incubation.percentage_startups_operational_after_3_yrs + '%')}
          ...
          <div className="col-span-2 py-2">
            ...
            <ul className="list-disc list-inside text-gray-200">
              {incubation.notable_alumni_startups.map((startup, index) => (
                <li key={index}>
                  {startup.startupName} -{" "}
                  {startup.websiteUrl && <a href={startup.websiteUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Website</a>}
                </li>
              ))}
            </ul>
          </div>
    ...
    const renderInvestorDetails = (investor: InvestorProfile) => (
      <>
        ...
        {renderDetailRow("LinkedIn", investor.linkedin_profile && <a href={investor.linkedin_profile} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Link</a>)}
    ...
    const renderMentorDetails = (mentor: MentorProfile) => (
      <>
        ...
        {renderDetailRow("LinkedIn", mentor.linkedin_profile && <a href={mentor.linkedin_profile} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Link</a>)}
        {renderDetailRow("City", mentor.city)}
        {renderDetailRow("Personal Website", mentor.personal_website && <a href={mentor.personal_website} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Link</a>)}
        ...
        <DialogContent className="max-w-3xl h-[80vh] flex flex-col p-6 bg-gray-900 text-white rounded-lg shadow-xl border border-gray-700">
          ...
          <ScrollArea className="flex-grow pr-4 -mr-4">
            <div className="py-4 space-y-4">
              {roleType === "startup" &&
                renderStartupDetails(profile as ApprovedStartup | PendingStartup)}
              {roleType === "incubation" &&
                renderIncubationDetails(profile as IncubationProfile)}
              {roleType === "investor" &&
                renderInvestorDetails(profile as InvestorProfile)}
              {roleType === "mentor" &&
                renderMentorDetails(profile as MentorProfile)}
            </div>
          </ScrollArea>
        </DialogContent>
      </Dialog>
